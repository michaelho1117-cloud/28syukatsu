...
const SIDEBAR_COPY = {
  events: { ja: 'イベント', en: 'Events', zh: '活动' },
  accounts: { ja: 'MyPage', en: 'MyPage', zh: 'MyPage' },
  researchHub: { ja: '研究资料', en: 'Research Hub', zh: '研究资料' }
};

function pickText(language, textSet) {
  if (language === 'zh') return textSet.zh;
  if (language === 'en') return textSet.en;
  return textSet.ja;
}

const getNavItems = (t, language) => [
  { path: '/dashboard', label: t('sidebar.dashboard'), icon: LayoutDashboard },
  { path: '/events', label: pickText(language, SIDEBAR_COPY.events), icon: CalendarCheck2 },
  { path: '/planner', label: t('sidebar.planner'), icon: ListTodo },
  { path: '/accounts', label: pickText(language, SIDEBAR_COPY.accounts), icon: User },
  ...
  { path: '/research-hub', label: pickText(language, SIDEBAR_COPY.researchHub), icon: LibraryBig },
