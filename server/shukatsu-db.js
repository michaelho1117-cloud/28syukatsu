...
function deriveNoteType(legacySourceType) {
  switch (String(legacySourceType || '').trim()) {
    case 'structured_note':
      return 'company_overview'
    case 'internship_rule':
...
function inferNoteTypeFromContent(title, tags, fallback = 'company_overview') {
  const text = `${String(title || '')} ${String(tags || '')}`.toLowerCase()

  if (
    text.includes('group-comparison') ||
  ...
  if (
    text.includes('decision summary') ||
    text.includes('判断材料') ||
  ...
  if (
    ...
    text.includes('es') ||
    text.includes('webテスト') ||
    text.includes('webtest') ||
    text.includes('case')
  ) {
    return 'selection_prep'
  }
...
function deriveSourceType(legacySourceType, currentSourceType) {
  const normalizedCurrent = String(currentSourceType || '').trim()
  if (RESEARCH_SOURCE_TYPES.includes(normalizedCurrent)) return normalizedCurrent
...
function migrateResearchAssetTaxonomy() {
  ensureColumn('CompanyResearchAsset', 'note_type', "TEXT NOT NULL DEFAULT 'company_overview'")

  const rows = db.prepare(`
    SELECT id, source_type, note_type, title, tags
  ...
  const tx = db.transaction(() => {
    for (const row of rows) {
      const hasModernSourceType = RESEARCH_SOURCE_TYPES.includes(String(row.source_type || '').trim())
      const normalizedNoteType = String(row.note_type || '').trim()
      const baseNoteType = hasModernSourceType && RESEARCH_NOTE_TYPES.includes(normalizedNoteType)
        ? normalizedNoteType
        : deriveNoteType(row.source_type)
      const nextNoteType = inferNoteTypeFromContent(row.title, row.tags, baseNoteType)
      const nextSourceType = deriveSourceType(row.source_type, row.source_type)

      if (nextNoteType !== row.note_type || nextSourceType !== row.source_type) {
        update.run({
          id: row.id,
...
function migrate() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS CompanyResearchAsset (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      company_id INTEGER NOT NULL,
      note_type TEXT NOT NULL DEFAULT 'company_overview',
  ...
  const assets = [
    {
      ...
      source_type: 'ai_generated',
        title: 'デロイト トーマツ グループ内ポジション比較メモ',
...
「最も大平台・主線感がある」のはDTFA。`
    ...
    {
      note_type: 'decision_material',
      source_type: 'ai_generated',
        title: 'デロイト トーマツ 志望判断サマリー',
