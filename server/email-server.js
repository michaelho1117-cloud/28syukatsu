...
async function runSync() {
  ...
  try {
    ...
    for await (const msg of client.fetch(`${start}:${end}`, { uid: true, envelope: true, source: true })) {
      ...
      const sourceText = Buffer.isBuffer(msg.source) ? msg.source.toString('latin1') : String(msg.source || '')
...
app.get('/api/email/body/:uid', async (req, res) => {
  const uid = Number(req.params.uid)
  if (!uid) return res.status(400).json({ ok: false, error: 'Invalid uid' })
  if (!cachedEmailConfig?.email || !cachedEmailConfig?.appPassword) {
    return res.status(400).json({ ok: false, error: 'Email config unavailable' })
  }
  ...
  try {
    ...
    for await (const msg of client.fetch(String(uid), { uid: true, source: true }, { uid: true })) {
      const sourceText = Buffer.isBuffer(msg.source) ? msg.source.toString('latin1') : String(msg.source || '')
      body = extractReadableBody(sourceText)
      break
    }
    ...
    if (!body) {
      return res.status(404).json({ ok: false, error: 'Readable body not found' })
    }
    ...
    db.prepare(`
      ...
      SET body_preview = ?
      WHERE uid = ?
    `).run(body.substring(0, 3000), uid)

    res.json({ ok: true, uid, body })
  } catch (error) {
    try {
      await client.logout()
    } catch {
